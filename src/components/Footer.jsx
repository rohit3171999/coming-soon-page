/*
* Description: The site footer containing social media, navigation links, and copyright info.
Purpose: Provide credibility, contact points, and legal information.
* */
export default function Footer() {
    // Define your social media links here for easy updates.
    const socialMedia = [
        {
            name: 'Twitter',
            ...
            <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                <div className="flex justify-center space-x-6">
                    {socialMedia.map((item) => (
                        <a key={item.name} href={item.href} className="text-gray-400 hover:text-blue-400 transition-colors duration-300">
                ...
                <nav className="mt-8 flex flex-wrap justify-center -mx-5 -my-2" aria-label="Footer">
                    {footerNavigation.map((item) => (
                        <div key={item.name} className="px-5 py-2">
                            <a href={item.href} className="text-base text-gray-400 hover:text-gray-200 transition-colors">
                                {item.name}
                            </a>
                        </div>
                    ))}
